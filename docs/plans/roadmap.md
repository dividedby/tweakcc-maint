...
| 51 | Onboard as an apply-agent-research Consumer loop | — | Blocked | human | `/apply-agent-research` | — | **Agent work DONE (2026-06-09)** — propose-only Consumer envelope vendored → `.github/workflows/apply-agent-research.yml` (downstream pattern from goodreads-bot: fetch-fresh skill+harness, hard-gate the leak guard, files through the skill's guarded `cli.py`, `contents:read`/`issues:write`, hash-slotted cron `27 3 * * 6` in the shared Fri-evening window, `--model sonnet`, harness `digest` cost-ledger line). Parameters filled: `CONSUMER_REPO=dividedby/tweakcc-maint`, `IS_KNOWLEDGE_SOURCE=no`, `DEFAULT_BRANCH=main`, `PRIVATE_MARKERS=""` (repo is public). Left **open, not merged** per the issue AC. **Human-only remaining (`ready-for-human`):** mint `SKILLS_TRACKER_TOKEN` (fine-grained PAT, Issues:rw on `dividedby/skills` only) + cost-ledger PAT (`Actions:Read` on this repo, handed to the agent-research owner as `TWEAKCC_ACTIONS_TOKEN`) + confirm `CLAUDE_CODE_OAUTH_TOKEN`, then merge. Cost-hub onboarding request filed on `dividedby/agent-research`. |
| 52 | Onboard to the architecture-review loop | — | Blocked | human | `/improve-codebase-architecture` | — | **Agent work DONE (2026-06-09)** — vendored `.github/workflows/improve-codebase-architecture.yml` (publish-seam envelope from goodreads-bot: fetch-fresh harness + skill from **`mattpocock/skills`**, read-only `gh`, deterministic `<output>`/`<body>` publish seam, hash-slotted cron `34 1 * * 6`, `--model sonnet`, cost-ledger line) **plus the load-bearing `.github/arch-review-context.md` include** (this repo's primary/fallback scope + binding disciplines: thin-cockpit/ADR 0001, orphan-authority/ADR 0005, no-build/ADR 0004, real/fake seams, the `.claude/hooks/` guards). **Verified (workflow_dispatch run 27192887804, green): filed deepening proposal #76, cost emitted.** Left **open, not merged** per the issue AC. **Human-only remaining (`ready-for-human`):** confirm `CLAUDE_CODE_OAUTH_TOKEN` (no cross-repo token needed), then merge. |
| 53 | Onboard to the staleness-review loop | — | Blocked | human | `/staleness-audit` | — | **Agent work DONE (2026-06-09)** — vendored `.github/workflows/staleness-review.yml` (publish-seam envelope from goodreads-bot: fetch-fresh harness + skill from `dividedby/skills`, `@SKILL_DIR@` substitution, **`WebSearch`+`WebFetch` granted** for the EOL/latest validate station, report-only — no `Edit`/`Write`, monthly **first-Monday-gated** cron `9 13 * * 1` + `first-monday-gate` job, `--model sonnet`, cost-ledger line). Scans this repo's pins (`.nvmrc`=22, `packageManager`/`engines`) ecosystem-general. **Verified (workflow_dispatch run 27192511665, green): filed report #75, cost emitted; report came back `unverified (no web access)` despite granted WebSearch/WebFetch — likely the `claude -p` + OAuth path, flagged for follow-up.** Left **open, not merged** per the issue AC. **Human-only remaining (`ready-for-human`):** confirm `CLAUDE_CODE_OAUTH_TOKEN` (no cross-repo token needed), then merge. |
