...
| **Ready (human / HITL)**          | 0 | — |
| **In review (PR open)**           | 1 | #207 (lobotomized realign → 2.1.172; draft PR lcc#9 for 2 of 6 opus-4-8 anchors) |
...
| 207 | Realign lobotomized overrides to CC 2.1.172 (gate `driver-check` fails — opus-4-8 inline anchors stale) | — | In review | human | `release-adoption` | — | HITL Release-adoption realignment (ADR 0002; PRD #196 back-half, out-of-loop). **Premise updated on scoping:** skrabe already realigned fable-5 + retired `inline-skill-keybindings-output-format.md` in both dirs (lcc `origin/main` eb3d6ad) after filing; no extractor defect and no new-prompt backlog — the 99 unnamed extractor items are ordinary Anthropic edits (the "23 content conflicts" framing was the unreproduced path). Remaining = the **opus-4-8 inline anchors** → leaf issue lcc#8 (2 mechanical re-anchors + 4 upstream restructures). **Draft PR lcc#9** prepared for the 2 mechanical (`intro`, `system-bullets`: anchor + mis-bind slot fixes `hqq→uJq`, `vMO→B6T`, each verified vs pristine 2.1.172, matches once). Partial — does **not** green the gate alone; the 4 restructures + the named-prompt half (needs a curated `prompts-2.1.172.json`) stay open with skrabe (cockpit rule, not merged) |
