...
| 26 | Leaf finding: lobotomized breaks CC 2.1.168 (evidence) | W1 | Backlog | human | — | — | **Un-parked** — live blocker for #58's lobotomized half. Pre-existing & version-independent: overrides ref slot names matching neither .168 nor .169 → boot `ReferenceError`. **Mechanical subset shipped** → skrabe/lobotomized-claude-code#5 (memory-synthesis `EMPTY_STRING`→`OPTIONAL_TAIL_NOTE`; enterplanmode-4-7 →`CONDITIONAL_WHAT_HAPPENS_NOTE_FN()`; escape latent `${VERSION}`/`${GITHUB_TOKEN}`/`${DATADOG_*}` examples). **Remainder open (curation / patcher altitude, `ready-for-human`):** restructured conditionals (croncreate-4-7 `${CRON_DURABLE_FLAG?…}`→`CRON_DURABILITY_SECTION`; workflow-4-8 5-slot rename) + inline-blob minified ids (`inline-*`, both 4-7/4-8 — need re-extraction, not a rename). Static pre-check only; HITL boot-verify is authority |
