...
**92 issues — 89 closed (97%), 3 open.**
**Closed (cumulative): 89.** ← integer total of all closed issues ever, including
...
| 234 | release-detector cron fails: Support matrix sourced from `claude --version` on a runner with no Claude Code (slice-1 regression) | — | Done | agent | `/tdd` | — | The every-4h `release-detector` cron had **never succeeded** (both scheduled runs failed): `release-detector-cli.ts` sourced the matrix from `RealAdoptionEnvironment.listMatrix()` (installed-version-only, `[claude --version]`, #22 gate split), but the detector runner installs no Claude Code → `claude --version` empty → throw (run 27380339912). Fix: install-free **hybrid** matrix — new `src/support-matrix.ts` (`SUPPORT_MATRIX_SEED` 2.1.169/170/172/173 ∪ pure `supportMatrix(proposalVersions)`) unioned with adopt-CC proposal titles read via `gh` (`listProposalVersions(repo,'all')`); `RealAdoptionEnvironment`/`claude --version` removed from the detector path. Unit test `test/support-matrix.test.ts`. Workflow-dispatch-verified green |
