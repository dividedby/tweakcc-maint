...
| **—**  | Cross-cutting / later | #11 #8 #80 #81 #76 #77 #75 | n/a |
...
| 81 | Index both skrabe leaf repos (tree + history + PRs) and synthesize into durable knowledge/skills/runbooks | — | Backlog | mixed | — | — | **Filed 2026-06-09.** One-time systematic index of `tweakcc-fixed` + `lobotomized` (current tree incl. `skills/showtime/`, full commit history, all PRs/issues) → synthesize per-finding into CONTEXT/ADR / skill / runbook / memory, with the no-synonyms terminology pass. Seed findings: the private third layer (`91df974` → `skrabes-claude-code` + bake-once overlay) + publish-hygiene rule + the canonical showtime skill. Read-only fan-out; size as a large research task. Needs-triage |
