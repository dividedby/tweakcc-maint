...
| 156 | Bump GitHub Actions off deprecated Node 20 (integration-gate + fallow-report @v4 → current majors) | — | Next | agent | — | — | GitHub forces Node 24 on 2026-06-16; the two CI workflows still pin `@v4` actions. Mechanical per-file bump per the issue's table; verify pnpm/action-setup v6 resolution + the gate still installs |
